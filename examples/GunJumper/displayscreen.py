...
""" Main menu screen """
    def __init__(self):
        ...
        self.selectbox = [12.5, 15, 205, 50] #coords of the box (x, y, width, height) which highlights selected item
        ...
        self.options = [(x, self.smallerfont.render(x, True, Screen.color["white"]).get_width()) #bind menu options to name and width
                        for x in ["New Game", "Load Game", "Options", "Credits", "Exit"]]
        self.maxwid = sorted(self.options, None, lambda x:x[1])[ - 1][1] # get the widest word
        
        def over(scr, mpos): #picks new location for gotobox
            wait = (mpos[1] - 13)//50
        ...
        def clickexit(someone, mpos): #ideally this will close the game
            print "AHAHAHA"
            #self.command = "exit"
        self.clickbox.append((30, 415, 240, 45), clickexit)
        self.command = False
