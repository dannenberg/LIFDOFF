...
def kick_player(addr, message):
    players_lock.acquire()
    if addr == str(host):
        for player in players: # TODO make this not a loop
            if str(player["name"]) == message:
                player["socket"][0].send("the host has kicked you")
                player["socket"][0].shutdown(SHUT_RDWR)
                player["socket"][0].close()
                while player["thread"].is_alive():
                    player["thread"]._Thread__stop() # we should make sure this is safe/necessary
                players[:] = [player for player in players if player["name"] != message]
        for player in players:
            player["socket"][0].send("player " + message + " kicked by host.")
    else:
        for player in players: # TODO make this not a loop
            if str(player["socket"][1]) == addr:
                player["socket"][0].send("only the game's host can kick players")
...
def send_message(addr, message):
    ...
    for player in players:
        if str(player["socket"][1]) != addr:
            player["socket"][0].send(sender + ": " + message)
    players_lock.release()
